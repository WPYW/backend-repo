...
def _lookup_link(document, keys):
    ...
    if not isinstance(keys, (list, tuple)):
        ...
        raise TypeError(msg)
    if any(
        [not isinstance(key, string_types) and not isinstance(key, int) for key in keys]
    ):
    ...
    if not isinstance(node, Link):
        ...
        raise exceptions.LinkLookupError(msg % (index_string, type(node).__name__))
...
class Client(itypes.Object):
    ...
    def get(self, url, format=None, force_codec=False):
        ...
        if format:
            force_codec = True
            decoders = [
                decoder for decoder in self.decoders if decoder.format == format
            ]
            if not decoders:
