...
def _key_sorting(item):
    ...
    if isinstance(value, Link):
        action_priority = {"get": 0, "post": 1, "put": 2, "patch": 3, "delete": 4}.get(
            value.action, 5
        )
...
class Document(itypes.Dict):
    ...
    def data(self):
        return OrderedDict(
            [(key, value) for key, value in self.items() if not isinstance(value, Link)]
        )

    @property
    def links(self):
        return OrderedDict(
            [(key, value) for key, value in self.items() if isinstance(value, Link)]
        )
    ...
    def get_messages(self):
        ...
        for value in self.values():
            if isinstance(value, Array):
                messages += [item for item in value if isinstance(item, string_types)]
        return messages
